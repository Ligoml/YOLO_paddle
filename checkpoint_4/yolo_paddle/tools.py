...
def loss(pred_conf, pred_cls, pred_txtytwth, label):
    ...
    cls_loss_function = nn.CrossEntropyLoss(axis=1, reduction='none')
    txty_loss_function = nn.BCEWithLogitsLoss(reduction='none')
    twth_loss_function = nn.MSELoss(reduction='none')

    pred_conf = paddle.nn.functional.sigmoid(pred_conf[:, :, 0]).astype("float32")
    pred_cls = paddle.transpose(pred_cls, perm=[0,2,1]).astype('float32')
    pred_txty = pred_txtytwth[:, :, :2].astype('float32')
    pred_twth = pred_txtytwth[:, :, 2:].astype('float32')
